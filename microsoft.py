...
def choose_subject(company):
    templates = ["Question for {Company}", "See this for {Company}", "Quick Question for {Company}"]
    return random.choice(templates).format(Company=company)

def check_reply(recipient_email):
    # replace with real IMAP/CRM logic
    return False
...
def send_email(to_addr, name, company, sender,
    ...
    try:
        with smtplib.SMTP(sender['smtp_server'], sender['smtp_port'], timeout=10) as server:
            ...
            print(f"{tag}{num} email sent to {to_addr}")
...
def send_emails(xlsx_path):
    ...
    # Create a pool sized to the number of recipients so nobody queues forever
    executor = ThreadPoolExecutor(max_workers=len(df))
    ...
    for _, row in df.iterrows():
        ...
        # Schedule both follow-ups immediately
        executor.submit(
            followup_scheduler,
